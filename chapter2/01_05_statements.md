...
前面由 throw 抛出的异常，通常需要进行捕获处理。try\/catch\/finally 语句即为异常处理机制。

通常，try 定义了需要处理的异常所在的代码块，catch 从句跟随在 try 从句之后，当 try 内某处发生异常，调用 catch 内的代码逻辑处理。catch 后面跟随 finally ，用于放置清理代码。**不管 try 中是否产生异常，finally 内的逻辑总是会被执行**。
